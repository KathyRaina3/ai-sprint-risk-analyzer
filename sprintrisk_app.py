...
# Display repository screenshots for context (optional, professional touch)
...
col1.image(img1, use_column_width=True, caption="Sprint Graph")
col2.image(img2, use_column_width=True, caption="Sprint Risk Dashboard")
col3.image(img3, use_column_width=True, caption="Progress Metrics")
...
if uploaded_file:
    ...
    fig, ax = plt.subplots()
    colors = ["red" if r == "High" else "orange" if r == "Medium" else "green" for r in risk_counts.index]
    ...
    # Optional: show static risk distribution image
    try:
        risk_img = Image.open("risk_distribution.png")
        st.image(risk_img, caption="Sample Risk Distribution", use_column_width=True)
    except:
        pass
